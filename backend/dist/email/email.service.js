...
export async function sendFinanceCounterpartyEmail(input) {
    if (env.NODE_ENV !== "production" && env.RESEND_API_KEY.startsWith("re_test")) {
        return;
    }
...
function buildLedgerEmailSummary(input) {
...
function getStatementRecords(input) {
    return input.statementRecords ?? input.openRecords ?? [];
}
function buildStatementFilename(ownerName, recipientName) {
