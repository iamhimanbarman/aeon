...
function drawStatementWordmark(doc, resources, x, y) {
...
function drawStatementBadge(doc, resources, x, y, size) {
    doc.save();
...
function drawAppBrandSeal(doc, resources, x, y) {
    const circleSize = 72;
    const circleX = x + 20;
    const circleY = y;
    doc.save();
...
function drawFallbackBrandSeal(doc, resources, x, y, size) {
    doc.font(resources.boldFont)
        .fontSize(28)
...
function drawPreparedForBlock(doc, input, resources, x, y) {
...
function drawInlineMetaItem(doc, resources, x, y, icon, text, width) {
...
function drawStatementDateCard(doc, resources, x, y) {
    doc.save();
...
function drawSummary(doc, input, resources, startY) {
...
function buildTotalCaption(totals) {
...
function drawLetter(doc, input, resources, startY) {
...
function drawRecordsTable(doc, input, resources, startY) {
    let y = startY;
    let index = 0;
    let pageIndex = 0;
...
function drawSectionHeading(doc, resources, x, y, title) {
...
function drawContinuationHeader(doc, resources, title) {
...
function drawTableFrame(doc, resources, x, y, width, height, rowCount) {
    const headerHeight = 38;
    doc.save();
...
function drawTableGrid(doc, x, y, width, height, rowCount, rowHeight) {
    const headerHeight = 38;
    let cursorX = x;
...
function drawDateCell(doc, resources, x, y, occurredAt, direction) {
...
function resolveStatementState(record, isNew) {
    if (isNew) {
        return {
            label: "New",
            accent: colors.gold,
...
function drawPillChip(doc, resources, x, y, text, fill, accent, minWidth = 66) {
...
function drawNetSummary(doc, input, resources, y) {
    const width = page.width - page.margin * 2;
    const totals = summarize(input.records);
...
function drawSoftShadow(doc, x, y, width, height, radius) {
    doc.save();
    doc.fillColor(colors.shadow).opacity(0.28);
    doc.roundedRect(x, y + 4, width, height, radius).fill();
    doc.restore();
}
function drawDotPattern(doc, x, y, columns, rows, xGap, yGap) {
    doc.save();
...
function drawBottomOrnament(doc, centerX, centerY) {
    doc.save();
...
function drawIconCircle(doc, x, y, radius, fill, accent, icon) {
...
function drawSmallLineIcon(doc, x, y, size, accent, icon) {
    const right = x + size;
    const bottom = y + size;
    const midX = x + size / 2;
    const midY = y + size / 2;
...
function drawLargeCardIcon(doc, x, y, size, accent, icon) {
    doc.save();
