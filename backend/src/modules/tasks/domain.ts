...
export type TaskShape = {
  ...
  domain: "general" | "study" | "work" | "health" | "finance" | "goal";
...
export function decodeTaskRecurrenceRule(value?: string | null): TaskRecurrenceRule | null {
  if (!value) {
    return null;
  }
  ...
  const frequency = parts[0] as TaskRecurrenceFrequency;
...
export function nextTaskOccurrence(task: TaskShape, completedAtIso: string): {
  dueAt?: string;
  reminderAt?: string;
} | null {
...
export function evaluateTaskIntelligence(task: TaskShape, nowIso = new Date().toISOString()): {
  score: number;
