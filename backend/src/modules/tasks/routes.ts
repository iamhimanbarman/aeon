...
export async function registerTaskRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/:taskId", { preHandler: app.authenticate }, async (request) => {
    const authUser = request.authUser!;
    return getTask(app.db, authUser.userId, (request.params as { taskId: string }).taskId);
  });

  app.get("/:taskId/subtasks", { preHandler: app.authenticate }, async (request) => {
    const authUser = request.authUser!;
    return listSubtasks(app.db, authUser.userId, (request.params as { taskId: string }).taskId);
  });

  app.get("/:taskId/reminders", { preHandler: app.authenticate }, async (request) => {
    const authUser = request.authUser!;
    return listReminders(app.db, authUser.userId, (request.params as { taskId: string }).taskId);
  });
  ...
  app.patch("/:taskId", { preHandler: app.authenticate }, async (request) => {
    ...
    return updateTask(app.db, authUser.userId, (request.params as { taskId: string }).taskId, body);
  });
  ...
  app.post("/:taskId/complete", { preHandler: app.authenticate }, async (request) => {
    ...
    return completeTask(app.db, authUser.userId, (request.params as { taskId: string }).taskId, body);
  });

  app.post("/:taskId/reopen", { preHandler: app.authenticate }, async (request) => {
    const authUser = request.authUser!;
    return reopenTask(app.db, authUser.userId, (request.params as { taskId: string }).taskId);
  });
  ...
  app.post("/:taskId/snooze", { preHandler: app.authenticate }, async (request) => {
    ...
    return snoozeTask(app.db, authUser.userId, (request.params as { taskId: string }).taskId, body.reminderAt);
  });

  app.delete("/:taskId", { preHandler: app.authenticate }, async (request, reply) => {
    const authUser = request.authUser!;
    await deleteTask(app.db, authUser.userId, (request.params as { taskId: string }).taskId);
    return reply.status(204).send();
  });
}
